...
ChildRunner.prototype = {
   ...
   */
  restartHost: function(id, overrides, callback) {
    debug('ChildRunner#restartHost 1');
    var remote = this._findRemote(id);
    debug('ChildRunner#restartHost 2', id, overrides);
    this.stopHost(id, function(err) {
      if (err) return callback(err);
      debug('ChildRunner#restartHost invoking _buildRemote');
      this._buildRemote(
        'build', // profile method
        'start', // host method,
        overrides,
    ...
    if (logServer) {
      try {
        ...
        debug('logServer.close failed', e.message);
  ...
  stopHost: function(id, callback) {
    debug("ChildRunner#stopHost 1");
    var remote = this._findRemote(id);
    debug(
      "ChildRunner#stopHost 2",
      typeof remote,
      remote && remote.host && remote.host.stop && String(remote.host.stop)
    );
    remote.host.stop(function(err) {
      debug("ChildRunner#stopHost remote.host.stop callback", err);
      if (err) return callback(err);
      remote.profileBuilder.destroy(callback);
    });
    debug("ChildRunner#stopHost 3");
  },
